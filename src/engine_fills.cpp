...
BacktestEngine::OrderEligibility BacktestEngine::classify_order_eligibility(
    ...
    // never matches a fill while dormant. Round 9 family X: the kill is
    // LEG-scoped — only the stop and limit legs die; a trail leg
    // (trail_points / trail_price, with or without an offset) keeps
    // resolving, so a dormant order that carries one stays eligible and
    // evaluate_fill_price masks its stop / limit (see
    // dormant_bracket_trail_leg_live).
    if (order.dormant_bracket && !dormant_bracket_trail_leg_live(order)) {
